...
if [ ! -d /www/server/panel/BTPanel ];then
	...
	echo "5.9平滑升级到6.0的命令：curl http://bt.79mz.cn/install/update_to_6.sh|bash"
	exit 0;
fi
...
if [ "${publicFileMd5}" != "${md5check}"  ]; then
	wget -O Tpublic.sh http://bt.79mz.cn/install/public.sh -T 20;
...
setup_path=/www
version=$(curl -Ss --connect-timeout 5 -m 2 http://bt.79mz.cn/api/get_version.php)
if [ "$version" = '' ];then
	version='7.4.5'
fi
...
#wget -O /www/server/panel/data/phplib.conf https://api.baotas.tech/ltd/install/lib/phplib.json
...
iptables -I INPUT -p tcp -m state --state NEW -m tcp --dport 2279 -j ACCEPT
service iptables save

firewall-cmd --permanent --zone=public --add-port=2279/tcp > /dev/null 2>&1
firewall-cmd --reload
...
echo "为了保障本机安全性，从现在起79版面板端口为:2279";
